...
export class BrandController {
  ...
  async downloadTemplate(
    @Query('example') example?: string,
    @Query('count') count?: string,
    @Query('format') format?: string,
    @Query('filename') filename?: string,
    @Query('sheet') sheet?: string,
    @Query('ext') ext?: string,
    @Query('withSample') withSample?: string,
    @Query('v') v?: string,
    @Query('q') q?: string,
    ...
    const { buffer, filename: tmplFilename } =
      await this.brandService.generateImportTemplate();
    res.setHeader(
      'Content-Type',
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="${tmplFilename}"`,
    );
    res.send(buffer);
  }
  ...
  async exportExcel(@Query() query?: any, @Res() res?: any) {
    const { buffer, filename } = await this.brandService.exportExcel(
      query || {},
    );
    res.setHeader(
      'Content-Type',
  ...
  @ApiQuery({
    name: 'search',
    required: false,
    description: 'Search in code, name, email, phone',
  })
